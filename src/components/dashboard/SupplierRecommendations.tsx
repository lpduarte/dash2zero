...
export const SupplierRecommendations = ({ suppliers }: SupplierRecommendationsProps) => {
  ...
  criticalSuppliers.forEach(critical => {
    ...
    if (alternatives.length > 0) {
      ...
      if (alternative.renewableEnergy > critical.renewableEnergy + 20) {
        benefits.push(`${(alternative.renewableEnergy - critical.renewableEnergy).toFixed(0)}% mais energia renovável`);
      }
      if (feImprovement > 20) {
